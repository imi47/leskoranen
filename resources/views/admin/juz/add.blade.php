		...
		<div class="col-md-12">
		    <div class="panel panel-bd">
		        ...
		        <div class="panel-body">
		            <form method="POST" action="{{ route('post-juz') }}" id="sub-form">
		            	@csrf
		            	<div class="row">
		            		<div class="col-md-6">
				            	<div class="form-group njuzname">
                          <label for="surah-name">Name (In Arabic)</label>
                          <input type="text" name="njuzname" class="form-control" id="surah-name" placeholder="e.g., الٓمّٓۚ">
                    ...
                    <div class="col-md-6">
                      <div class="form-group njuznumber">
                          ...
                          <input type="number" name="njuznumber" class="form-control" id="juz-num" placeholder="e.g., 1">
                      ...
                      <div class="form-group nstartsurah">
                          ...
                          <select class="form-control basic-single" id="start-surah" name="nstartsurah">
                            ...
                            @foreach($surahs as $key => $row)
                              <option value="{{ $row->id }}" surah_number="{{ $row->surah_number }}" id="verses-op{{ $row->id }}" varses="{{ $row->verses }}">{{ $row->surah_number . '. ' . $row->surah_name }}</option>
                            @endforeach
                          </select>
                    ...
                    <div class="col-md-3">
                      <div class="form-group nendsurah">
                          ...
                          <select class="form-control basic-single" id="end-surah" name="nendsurah">
                            ...
                            @foreach($surahs as $key => $row)
                              <option value="{{ $row->id }}" end_surah_number="{{ $row->surah_number }}" id="end-verses-op{{ $row->id }}" varses="{{ $row->verses }}">{{ $row->surah_number . '. ' . $row->surah_name }}</option>
                            @endforeach
                          </select>
